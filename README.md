![Full Stack Web Development Internship](https://capsule-render.vercel.app/api?type=waving&color=0:0077B5,100:00C6FF&height=200&section=header&text=SuprMentr%20FSWD%20Internship&fontSize=35&fontColor=ffffff&animation=fadeIn)

# 💻 SuprMentr Full Stack Web Development Internship

## Internship Assignments Repository

------------------------------------------------------------------------

## 📌 Repository Overview

This repository contains all the **assignments, tasks and projects**
completed during the **Full Stack Web Development Internship at
SuprMentr**.

The purpose of this repository is to maintain a **structured record of
internship work**, demonstrate the **practical implementation of web
development concepts** and **track learning progress** throughout the
internship program.

Since the internship is **currently ongoing**, new assignments, tasks 
and projects will continue to be added as the learning journey
progresses.

------------------------------------------------------------------------

## 👨‍💻 Intern Information

**Name:** Sayed Jahangir Ali\
**USN:** 1AM22IS096\
**College:** AMC Engineering College\
**Internship Program:** Full Stack Web Development Internship\
**Platform:** SuprMentr

**GitHub Profile:**\
https://github.com/Jahangir-ali-74

------------------------------------------------------------------------

## 📂 Repository Structure

The repository contains various folders and files corresponding to the
**assignments and tasks completed during the internship**.

Typical contents include:

-   Internship Assignments.
-   Practical Coding Tasks.
-   Mini Projects.
-   Documentation Files.
-   Internship Work Records.

Each folder represents a **specific assignment or topic covered during
the internship**.

------------------------------------------------------------------------

## 🎯 Purpose of this Repository

This repository serves as:

-   📘 A record of internship assignments.
-   💼 A portfolio of practical projects.
-   📈 A learning progress tracker.
-   🔍 A reference for web development concepts.

------------------------------------------------------------------------

## 🔗 Repository Link

You can explore the full repository here:

https://github.com/Jahangir-ali-74/SAYED-JAHANGIR-ALI-1AM22IS096-FSWD-SuprMentr-Internship-Assignments

------------------------------------------------------------------------

## 📅 Internship Status

🟢 **Status:** Ongoing

Assignments, tasks and projects will continue to be added as part of
the internship learning process.

------------------------------------------------------------------------

## 📌 Conclusion

This repository showcases my **learning journey and practical work**
completed during the **SuprMentr Full Stack Web Development
Internship**. It reflects my understanding of **modern web development
concepts** and my ability to apply them through **hands-on assignments
and projects**.

------------------------------------------------------------------------

## 👤 Author

Created by **[Sayed Jahangir Ali](https://github.com/Jahangir-ali-74)**

© 2026
